...
xp = tf.placeholder(tf.float32, [batch_size, img_size, img_size, filter_in_channel])
...
with tf.variable_scope(cl1_layer_name):
    ...
    conv1 = tf.nn.conv2d(xp,conv1_weight,strides=[1,1,1,1],padding='SAME')
    ...
    layer1_conv = activate_func(layer1_conv)
...
with tf.variable_scope(cl3_layer_name):
    ...
    layer3_conv = activate_func(layer3_conv)
...
with tf.variable_scope(cl5_layer_name):
    ...
    layer5_conv = activate_func(layer5_conv)
