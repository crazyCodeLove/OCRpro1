...
from mnistCompResv4.mnisttool import ModelUtilv4
from mnistCompResv4.mnisttool import imgUtil
from mnistCompResv4.mnisttool import mnistModel
...
logger = ModelUtilv4.MyLog('/home/allen/work/data/resultlog/mnistCompResv4/mnistCmpResn1.txt')
logDir = '/home/allen/work/data/resultlog/mnistCompResv4/summary/cmpres4'
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for itstep in range(base_step,end_step):
            ...
            if itstep % 100 == 0:
                ...
                trainacc = ModelUtilv4.get_accurate(outprediction, inlabels)
        ...
        print("after save")
        ModelUtilv4.update_peizhi(gps.peizhi_filename,'is_restore',True)
        ModelUtilv4.update_peizhi(gps.peizhi_filename,'train_step',end_step)
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for itstep in range(base_step, end_step):
            ...
            (inlabels, outprediction) = sess.run(
                ...
                feed_dict=feed_dict)
            itrightnums = ModelUtilv4.get_test_right_num(outprediction, inlabels)
            allrightnums += itrightnums


        test_acc = float(allrightnums)/10000
        lrn_rate = ModelUtilv4.down_learning_rate(test_acc,lrn_rate)
        ModelUtilv4.update_peizhi(gps.peizhi_filename,'lrn_rate',lrn_rate)
        ModelUtilv4.update_peizhi(gps.peizhi_filename,'test_step',end_step)

        if test_acc > peizhi['max_test_acc']:
            ModelUtilv4.update_peizhi(gps.peizhi_filename,'max_test_acc',test_acc)
            ModelUtilv4.move_variable_from_src2des(gps.save_dirname, gps.des_save_dirname)
...
def batch_imgs_process_train(batch_imgs, hps):
    ...
    batch_imgs = imgUtil.batch_imgs_pre_process(batch_imgs)
    return batch_imgs
...
def main():
    ...
    des_save_dirname = '/home/allen/work/variableSave/OCRpro1/compresv4'
    ...
    gps = GParams(save_file_name=save_file_name,
                  ...
                  peizhi_filename='/home/allen/work/chengxu/OCR/OCRpro1/mnistCompResv4/peizhi.xml'
                  )

    ModelUtilv4.init_peizhi(
