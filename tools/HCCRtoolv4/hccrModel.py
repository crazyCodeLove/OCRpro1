...
from tools.HCCRtoolv4 import ModelUtilv4


class HCCRModel(object):
    ...
    def create_deep_res_head(self, inputx, is_training_ph, activateFunc=tf.nn.relu):
        ...
        outputs = ModelUtilv4.add_BN_conv_layer(
    ...
    def create_deep_res_body(
        ...
        for it in range(len(carriage_block_num)):
            # if it == 0:
            #     outputs = ModelUtilv4.add_overlap_maxpool(outputs)
            # else:
            #     outputs = ModelUtilv4.add_averagepool_layer(outputs)
            outputs = ModelUtilv4.add_maxpool_layer(outputs)

            tscope = "carriage_" + str(it)
            outputs = ModelUtilv4.add_building_block_carriage(
            ...
            outputs = ModelUtilv4.building_block_desc(
    ...
    def create_resnet(self, inputx, is_training_ph, activateFunc=tf.nn.relu):
        ...
        # outputs = ModelUtilv4.add_fractional_maxpool(outputs,ratio=[1,1.4,1.4,1])
    ...
    def create_graph(self):
        ...
        fcl1_inputs, fcl1_in_features = ModelUtilv4.conv2fc(resnet)
        # set outputs features
        outputs_features = self.hps.num_classes
        outputs = ModelUtilv4.add_fc_layer(fcl1_inputs, fcl1_in_features,
                                                         outputs_features)
