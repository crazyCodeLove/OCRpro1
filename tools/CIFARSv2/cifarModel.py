...
from tools.CIFARSv2 import ModelUtilv3s1
...
class CifarModel(object):
    ...
    def create_deep_res_head(self, inputx, is_training_ph, activateFunc=tf.nn.relu):
        ...
        with tf.name_scope("res_head"):
            ...
            # tf.summary.image('res_head',outputs[:,:,:,:1])
    ...
    def create_deep_res_body(
        ...
        with tf.name_scope("res_body"):
            ...
            for it in range(len(carriage_block_num)):
                ...
                with tf.name_scope(tscope):
                    outputs = ModelUtilv3s1.add_building_block_carriage(
                        batch_num, self.hps.deepk, carriage_block_num[it], outputs,
                ...
                # tname = "carriage_"+str(it)
                # tf.summary.image(tname,outputs[:,:,:,:1])
            return outputs
