...
def get_accurate(prediction,labels):
    return tf.reduce_mean(tf.to_float(tf.equal(tf.arg_max(prediction,1),tf.arg_max(labels,1))))


def get_test_right_num(prediction,labels):
    return tf.reduce_sum(tf.to_float(tf.equal(tf.arg_max(prediction,1),tf.arg_max(labels,1))))
...
def building_block_incre(batch_num,deepk,inputs,kernalWidth,
    ...
    for it in range(deepk-1):
        proj = tf.concat(values=[proj,zero_pad],axis=3)
